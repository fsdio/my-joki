...
if Trim(edit_idapotek.Text) = '' then
  ...
  if MyDataModule.get_jadwal_.Locate('id_apotek',edit_idapotek.Text,[]) then
    ...
    begin
      ...
      begin
        get_jadwal_.SQL.Text := 'UPDATE `tb_apotek` SET nama_obat=:setNamaObat,harga=:setHarga,qty=:setQTY,tgl_produksi=:setTglProduksi,indikasi=:setIndikasi,jenis=:setJenis WHERE id_apotek=:setId';
        ...
        get_jadwal_.ParamByName('setTglProduksi').Value   := edit_tanggal.DateTime;
...
begin
  ...
  if MyDataModule.get_jadwal_.Locate('id_apotek',edit_idapotek.Text,[]) then
    ...
    begin
      with MyDataModule do
      begin
        get_jadwal_.SQL.Text := 'INSERT INTO `tb_apotek` VALUES (:setId,:setNamaObat,:setHarga,:setQTY,:setTglProduksi,:setIndikasi,:setJenis)';
        get_jadwal_.ParamByName('setId').Value            := edit_idapotek.Text;
        get_jadwal_.ParamByName('setNamaObat').Value      := edit_nameobat.Text;
        get_jadwal_.ParamByName('setHarga').Value        := edit_harga.Value;
        get_jadwal_.ParamByName('setQTY').Value           := edit_qty.Value;
